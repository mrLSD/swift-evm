        ...
        describe("overflowAdd") {
            ...
            context("when adding two small numbers without carry") {
                it("returns correct sum without overflow") {
                    ...
                    let expectedSum = U256(from: Array(repeating: UInt64(0xA + 0xE), count: Int(U256.numberBase)))
            ...
            context("when adding numbers: (MAX-1) +1") {
                it("returns correct sum without overflow") {
                    let a = U256(from: [UInt64](repeating: UInt64.max - 1, count: Int(U256.numberBase)))
