...
final class InstructionOrSpec: QuickSpec {
    static var machine: Machine {
        return TestMachine.machine(opcode: Opcode.OR, gasLimit: 10)
    }

    static var machineLowGas: Machine {
        return TestMachine.machine(opcode: Opcode.OR, gasLimit: 2)
    }
    ...
    override class func spec() {
        describe("Instruction OR") {
            it("a | b") {
                ...
                _ = m.stack.push(value: U256(from: 8))
                ...
                expect(result).to(beSuccess { value in
                    expect(value).to(equal(U256(from: 0xC)))
                })
